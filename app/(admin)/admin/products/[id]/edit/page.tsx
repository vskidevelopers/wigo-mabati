...
interface ImageUpload {
    ...
    file?: File
    url: string
    status: 'existing' | 'uploading' | 'success' | 'error'
    error?: string
}
    ...
    useEffect(() => {
        async function fetchProduct() {
            try {
                ...
                setBaseUnit(product.base_unit ?? 'meter')
                setIsActive(product.is_active ?? true)
                setSelectedColors(product.available_colors || [])
                setSelectedGauges(product.available_gauges || [])
                setSelectedFinishes(product.available_finishes || [])

                const rawPricePerGauge = product.price_per_gauge
                let parsedPricePerGauge: Record<number, number> = {}

                if (typeof rawPricePerGauge === 'string') {
                    try {
                        const parsed = JSON.parse(rawPricePerGauge)
                        if (typeof parsed === 'object' && parsed !== null && !Array.isArray(parsed)) {
                            parsedPricePerGauge = parsed as Record<number, number>
                        }
                    } catch {
                        parsedPricePerGauge = {}
                    }
                } else if (typeof rawPricePerGauge === 'object' && rawPricePerGauge !== null && !Array.isArray(rawPricePerGauge)) {
                    parsedPricePerGauge = rawPricePerGauge as Record<number, number>
                }

                setPricePerGauge(parsedPricePerGauge)

                // Parse existing images
                if (product.image_url) {
                    try {
                        const imageUrls: string[] = JSON.parse(product.image_url)
                        setImages(
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const imageUrls = images
                .filter(img => img.url && img.status !== 'error')
                .map(img => img.url)
                                ...
                                {images.map((image) => (
                                    <div key={image.id} className="relative group">
                                        ...
                                        <div className="absolute top-2 right-2">
                                            {(image.status === 'existing' || image.status === 'success') && (
                                        ...
                                        <div className="mt-2 flex items-center justify-between">
                                            <p className="text-xs text-muted-foreground truncate flex-1">
                                                {image.status === 'existing' ? 'Existing image' : image.file?.name}
                                            </p>
                                            <Button
                                                type="button"
                                                variant="ghost"
